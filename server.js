...
// Fallback: also serve root index.html if needed
app.get("/", (req, res) => {
...
function deal(room) {
  ...
  deck.forEach((card, i) => {
    room.players[i % room.players.length].cards.push(card);
  });

  room.players.forEach(p => {
    p.cards.sort((a, b) => a.value - b.value);
  });
...
io.on("connection", socket => {
  ...
  socket.on("thola", () => {
    ...
    if (room.pile.length === 0) {
      return socket.emit(
        "error:message",
        "There are no cards in the center."
      );
    }

    const highest = room.pile.reduce(
      (a, b) => b.value > a.value ? b : a
    );

    const winner =
      room.players.find(p => p.id === highest.playerId);

    if (!winner) return;

    const taken = room.pile.map(c => ({
      id: c.id,
      rank: c.rank,
      value: c.value,
      suit: c.suit,
...
server.listen(
  ...
  () => {
    console.log(
      `Bhabhi Thola running on port ${PORT}`
    );
  }
);
